...
export default function AdminLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.requiresMfa) {
        router.push('/admin-auth/login/mfa');
      } else {
        router.push('/admin');
      }
    } catch {
    ...
    <main className="min-h-screen bg-muted/30 flex items-center justify-center p-6">
      <div className="w-full max-w-sm space-y-6 rounded-lg border border-border bg-background p-8 shadow-sm">
        <div>
          ...
          <h1 className="mt-2 text-2xl font-bold">Admin Sensident</h1>
          <p className="text-sm text-muted-foreground">RÃ©servÃ© Ã  l'Ã©quipe Ã©ditoriale.</p>
        </div>
